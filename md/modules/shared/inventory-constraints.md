...
**NotUsingItem** — Requires that the player is not currently using an item (eating, drinking, drawing a bow, blocking with a shield, etc.). Actions are postponed until the item use finishes.

**NotBreaking** — Requires that the player is not currently breaking a block. Actions are postponed while a block is being mined.

**NotDuringCombat** — Requires that the player is not in combat, according to the client's combat tracker. Actions are postponed while you are fighting.
