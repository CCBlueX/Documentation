...
Because freezing your packets can desync you from the server, you may get teleported back ("flagged") when the connection catches up. Use **DisableOn** to pick the situations in which the module switches itself off — being flagged, standing on the ground, pressing a movement key, entering liquid, falling into the void or using an item — and turn on **BalanceWarp** if you want the held-up movement to be replayed smoothly when you disable Freeze instead of being thrown away.
...
| DisableOn | Multi-Select | Flag | Flag, OnGround, OnMovementInput, InLiquid, Void, OnUseItem | Conditions that automatically turn Freeze off: the server teleports you back (Flag), you stand on the ground, you press a movement key, you enter liquid, you drop below the world (Void), or you use an item. |
| Notification | Toggle | false | — | Shows an on-screen notice naming the condition that made Freeze disable itself. |
