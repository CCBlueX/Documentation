...
It offers two modes. **Fancy** is the smarter, continuous option: while you press into a wall (or hold jump/sneak against a ceiling or floor), it scans for the nearest open space on the other side and clips you there, repeating as long as you keep moving. By default it only clips into air, but **AllowedLiquids** lets you also treat water and/or lava as valid destinations. It also shows ▲/▼ markers next to your crosshair to indicate which vertical clips are currently available, and it stays out of the way while [Fly](/docs/modules/movement/fly) is active. **Old** is a one-shot teleport: the moment you enable the module it jumps you a fixed horizontal and vertical distance based on where you're facing, then turns itself back off.
...
| Choice → [Fancy] → Vertical | Integer | 5 | 0..10 | Maximum number of blocks to clip up or down when holding jump or sneak. |
| Choice → [Fancy] → AllowedLiquids | Multi-Select | None | Water, Lava | Liquids to treat as valid clip destinations in addition to air. |
...
---
*Last updated: 2026-07-27 — Based on [source code](https://github.com/CCBlueX/LiquidBounce/blob/c8913426374/src/main/kotlin/net/ccbluex/liquidbounce/features/module/modules/exploit/ModuleClip.kt)*
