...
- **ThrowableType** (Choice) — default: `EGG_AND_SNOWBALL`; options: `EggAndSnowball`, `Anything` — Determines which throwable items to use for shooting.
...
- **AimOffThreshold** (Decimal) — default: `2.0`; range: `0.5` – `10.0` — Maximum angle difference in degrees between server and client rotation before shooting.
...
- **ConstantLag** (Toggle) — default: `false` — Adjusts behavior for high-lag servers.
