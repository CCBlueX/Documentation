# Anticheats

This page lists which anticheat is known to be used on popular servers. Knowing the anticheat of a server helps you choose a fitting config.

### Polar

| Server | Anticheat |
|---|---|
| mc.jartexnetwork.com | Polar |
| mc.universocraft.com | Polar |
| mc.fakepixel.fun | Polar |
| play.minemalia.com | Polar |
| play.trexmine.com | Polar |
| mc.masedworld.ru | Polar |
| mc.mineblaze.net | Polar |
| mc.mineberry.org | Polar |
| gommehd.net | Polar |
| play.ultimismc.com | Polar |
| mc.agerapvp.club | Polar |
| twerion.net | Polar |
| mc.hycraft.us | Polar |
| cheatmine.ru | Polar |
| gamster.org | Polar |
| mustpvp.pl | Polar |
| revage.net | Polar |
| dexland.ru | Polar |
| coralmc.it | Polar |
| pika.host | Polar |
| pvp.land | Polar |
| arch.mc | Polar |
| bridger.land | Polar |
| go.minepeak.org | Polar |
| seiky.net | Polar |

### Intave / Intave forks

| Server | Anticheat |
|---|---|
| mc.mineland.net | Old custom Intave (nemesis) |
| mc.hypemc.pro | Old Intave (14.9.0) |
| craftplay.pl | Old Intave |

### Grim / Grim forks

| Server | Anticheat |
|---|---|
| blocksmc.com | Grim + Verus + NCP |
| mc.musteryworld.net | Grim + Matrix |
| teslacraft.org | Grim + Vulcan |
| play.pvplegacy.net | Grim V3 |
| play.mccisland.net | Grim V3 |
| mc.gamertee.net | Grim |
| funnymc.su | Grim + Matrix |
| mc.zentic.cc | Bad Grim |
| mc.minetoday.org | Bad Grim |
| luckynetwork.net | Grim |
| mc.vimemc.net | Grim |
| hoplite.gg | Grim V3 |
| catpvp.xyz | Grim V3 |
| zonecraft.es | Grim |
| triadamc.net | Grim |
| hylexmc.net | Grim |
| mcpvp.club | Grim |
| 3fmc.com | Grim + Matrix |
| hylex.gg | Grim |
| kaizenmc.gg | Grim |
| astrummc.net | Old custom Grim |

### Vulcan

| Server | Anticheat |
|---|---|
| deathzone.net | Vulcan |
| mc.minelatino.com | Vulcan + Grim + unknown |
| play.regorland.net | Bad Vulcan |
| supercraft.fun | Vulcan |
| nerdcats.asia | Vulcan |
| macesmp.net | Vulcan |
| teamholy.de | Vulcan |
| crack.pakmc.xyz | Vulcan |

### Verus

| Server | Anticheat |
|---|---|
| librecraft.com | Old Verus + NCP |
| np.coldpvp.com | Verus Custom |
| sololegends.net | Verus |
| pvpgym.net | Verus |

### Matrix

| Server | Anticheat |
|---|---|
| mc.mineruby.org | Old Matrix |
| fcraft.su | Matrix |
| mlegacy.net | Matrix + NCP |
| funnymc.ru | Matrix + NCP |
| play.1mine.ru | Matrix |
| mc.skycave.pro | Matrix |
| spotymc.ru | Matrix |

### HvH

| Server | Anticheat |
|---|---|
| mc.themoskau.xyz | Old NCP |
| duels.nezha.ac | 6+ anticheats |

### Other

| Server | Anticheat |
|---|---|
| hypixel.net | Watchdog-Prediction |
| mushmc.com.br | Mush-Prediction |
| cubecraft.net | Sentinel |
| minemen.club | AGC |
